...
export default function HomePage(){
  ...
  return (<div className="py-8">
    <section className="rounded-2xl overflow-hidden border border-gray-800 mb-8">
      <div className="h-[240px] md:h-[360px] w-full bg-cover bg-center" style={{backgroundImage:`url(${city.heroImage})`}}/>
      <div className="p-6 md:p-8 bg-gradient-to-b from-black/20 to-black/60">
        <h1 className="text-3xl md:text-4xl font-bold mb-2">{city.city}, {city.state} — Local News & Events</h1>
    ...
    <section className="grid md:grid-cols-3 gap-6">
      ...
      <div className="card">
        ...
        <p className="text-gray-300 text-sm">Post daily updates under <b>/news</b>. You can automate later.</p>
        <a href="/news" className="nav-link mt-2 inline-block">Go to News →</a>
      </div>
      <div className="card">
        <h2 className="text-xl font-semibold mb-3">Advertise</h2>
        <p className="text-gray-300 text-sm">Promote events or sponsor posts across this city, or bundle multiple cities.</p>
    ...
    <section className="card mt-6">
      ...
      <ul className="grid md:grid-cols-2 gap-2 mt-3 text-sm">
        {["saltlakeut.com","irmosc.com","caycesc.com","elizabethnc.com","fresnoca.org","indioca.com","kahuluihi.com","perrisca.com"].map(h=>(
          <li key={h}><a className="nav-link" href={`https://${h}`} target="_blank" rel="noreferrer">{h}</a></li>
        ))}
      </ul>
    </section>
  </div>);
